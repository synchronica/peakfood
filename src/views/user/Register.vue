        ...
        <div class="form-side">
          ...
          <template v-if="formSent === false">
            ...
            <b-form @submit.prevent="formSubmit">
              ...
              <label class="form-group has-float-label mb-4">
                <input type="email" trim class="form-control" v-model="email" required>
                <span>{{ $t('user.email') }}</span>
              </label>

              <label class="form-group has-float-label mb-4">
                <b-form-input type="phone" trim class="form-control" v-model="phoneNumber" required />
                <span>Telefono</span>
              </label>
              ...
              <div class="d-flex justify-content-end align-items-center">
                <b-button v-if="loading === false" type="submit" variant="primary" size="lg" class="btn-shadow">
          ...
          <div v-else-if="formSent === false && errorOcurred" class="text-center">
  ...
  methods: {
    async formSubmit () {
      if (this.valid) {
        ...
        formData.set('displayName', this.fullname)
        formData.set('email', String(this.email))
        formData.set('phoneNumber', this.phoneNumber)
        formData.set('password', this.password)

        let hostname = location.hostname === 'localhost' ? localHost : location.hostname
        let uid
        try {
          const response = await axios.post(`https://${hostname}/app/v3/api/?createUser`,
            formData,
            {
              headers: {
        ...
        try {
          const response2 = await axios.post(`https://${hostname}/app/v3/api/?sendEmailVerification`,
            formData2,
            {
              headers: {
                'Content-Type': 'multipart/form-data'

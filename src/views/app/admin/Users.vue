    ...
    <b-colxx>
      ...
      <template v-if="isLoad">
        <b-row v-if="displayMode === 'list'" key="list">
          <b-row v-if="users.length < 1" class="justify-content-center w-100">
...
export default {
  ...
  methods: {
    async getUsers() {
      ...
      try {
        const response = await this.$store.state.axios.get("/api/GetUsers");
        console.log("getUsersResponse", response);

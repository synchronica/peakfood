        ...
        >
          <template v-for="(field, index) in localFields">
            ...
            >
              ...
              <!-- <div v-else-if="isCestino" class="w-sm-100 mind-icons" @click="toggleFavorite">
              <i style="font-size: 24px; cursor: pointer; color:green;" class="glyph-icon" :class="{'icon-heart': data.favorite == 0, 'icon-heart1': data.favorite == 1}"/>
              </div>-->
            ...
            <div
              ...
              class="w-sm-100 d-flex flex-grow-1 justify-content-end"
              :key="index"
              :class="`w-10`"
            >
              <template v-for="action in actions">
                ...
                <div
                  v-if="action === 'delete' || action === 'softDelete'"
                  class="pl-1 align-self-center mx-2"
                  :key="action"
                >
...
export default {
  ...
  methods: {
    ...
    openDistintaBase() {
      const payload = {
        id: this.data.id,
        recipeId: this.data.market_recipe_id,
        item: this.data,
    ...
    showHeader({ field, type }) {
      if (field === "ingredients") {
        return false;
      } else if (type === "excluded" || type === "hidden") {
        return false;
    ...
    showField(field) {
      if (field.type.includes("excluded") || field.type.includes("hidden")) {
        return false;
      }

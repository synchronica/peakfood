    ...
    <div class="pl-2 d-flex w-100 min-width-zero">
      <div class="card-body w-100 w-sm-100 align-self-center d-flex flex-column flex-lg-row align-items-lg-center">
        <template v-for="(field, index) in localFields">
          <div v-if="Number(field.in_list) > 0 && data[field.field] !== null" class="w-sm-100" :class="getWidth(index, field)" :key="field.field + field.id">
            <!-- MOBILE HEADER -->
        ...
        <!-- ROW ACTIONS -->
        <div v-if="actions" class="w-10 w-sm-100 d-flex flex-grow-1 justify-content-end flex-lg-row" :key="index">

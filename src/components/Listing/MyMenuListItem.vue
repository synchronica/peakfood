        ...
        <div style="padding-left:24px" class="card-body pt-0 align-self-center d-flex flex-column flex-lg-row">
          <template v-for="(field, lIndex) in localFields">
            <div class="d-none d-lg-block d-xl-block text-truncate" :class="getHeaderWidth(lIndex)" :key="field.field">
              ...
              <span v-else-if="field.field === 'last_sold_total'">
                Dato di vendita
              </span>
    ...
    <div class="pl-2 d-flex w-100 min-width-zero">
      <div class="card-body w-100 w-sm-100 align-self-center d-flex flex-column flex-lg-row align-items-lg-center">
        <template v-for="(field, index) in localFields">
          <div v-if="Number(field.in_list) > 0 && data[field.field] !== null" class="w-sm-100 mb-sm-2" :class="getWidth(index)" :key="field.field + field.id">
            <!-- MOBILE HEADER -->
            <span v-if="field.field !== 'actions' && field.field !== 'categoria_vendita'" class="d-lg-none d-xl-none">{{ capitalize(field.field) }}</span>

            <!-- FIRST ICON OF SALES CLASS -->
            <div v-if="field.field === 'sales_class'" style="width: 5%" class="w-sm-100 mind-icons">
            ...
            <!-- NORMAL BADGE -->
            <div v-else-if="field.background && field.field !== 'actions' && field.action !== 'fileOpen' && field.field !== 'categoria_vendita'" class="w-sm-100">
            ...
            <b-form-input v-else-if="field.field === 'last_sold_total'" @change="onChange" v-model.number="lastSoldTotal"/>

            <p v-else-if="field.field === 'margine'" class="mb-1 text-muted text-small w-sm-100" :key="field.field + index">{{ field.value }}</p>

            <!-- TEXT ITEMS IN LIST -->
            <p v-else-if="field.action !== 'fileOpen' && field.field !== 'favorite' && field.type !== 'selectText' && field.field !== 'actions'" class="mb-1 text-muted text-small w-sm-100" v-html="decodeHTML(data[field.field])"/>
          </div>

          <div v-if="field.field === 'actions'" class="w-sm-100 d-flex justify-content-end" :key="index">
...
export default {
  ...
  computed: {
    localFields () {
      ...
      let fieldsSorted = fields.sort((a, b) => (Number(a.in_list) > Number(b.in_list)) ? 1 : ((Number(b.in_list) > Number(a.in_list)) ? -1 : 0))
      return fieldsSorted
    }
  },
  created () {
